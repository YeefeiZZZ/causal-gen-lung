...
from svp.mimic.datasets import create_dataset
from svp.mimic.train import create_model_and_optimizer, _LabelSmoothing
...
def coreset(run_dir: str = './run',
            ...
            datasets_dir: str = './data', dataset: str = 'mimic_cf',
            augmentation: bool = True,
            validation: int = 0, shuffle: bool = False,
    ...
    """
    Perform core-set selection on MIMIC.
    ...
    if precomputed_selection is None:
        ...
        train_indices, dev_indices = utils.split_indices(
            train_dataset, validation, proxy_run_dir, shuffle=False)
        ...
        with open(run_dir + '/forgetting_rates.json', 'w') as file:
            json.dump(nevents.tolist(), file)
    ...
    # print(type(target_train_indices))
    file_path =[sample[0] for sample in train_dataset.samples]
    selected_path = []
    for idx in range(len(target_train_indices)):
        selected_path.append(file_path[target_train_indices[idx]]) 
    
    with open(run_dir + '/selected_path.json', 'w') as file:
        json.dump(selected_path, file)
