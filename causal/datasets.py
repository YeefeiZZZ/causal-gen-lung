...
class MIMICMetadata(TypedDict):
    ...
    race: int  # 0 -> white , 1 -> black, 2 -> asian
...
def read_mimic_from_df(
    ...
    meta = MIMICMetadata(age=age, wcc=wcc, sex=sex, race=race)
...
class MIMIC(Dataset):
    def __init__(
        ...
        # if self.columns is None:
            # ['eid', 'sex', 'age', 'brain_volume', 'ventricle_volume', 'mri_seq']
        self.columns = [ 'age', 'wcc','race_label','sex_label','disease_label']
...
def preprocess_mimic(sample: Dict[str, Tensor]) -> Dict[str, Tensor]:
    for k, v in sample.items():
        if k != "x":
            ...
            if k == "race":
                sample[k] = F.one_hot(sample[k].to(torch.int64), num_classes=3).squeeze()
            elif k == "finding":
                sample[k] = F.one_hot(sample[k].to(torch.int64), num_classes=3).squeeze()
            elif k == "age":
                ...
                # print(sample[k],sample[k] / 100 * 2 - 1, inv_preprocess_t(sample[k] / 100 * 2 - 1))
                # sample[k] = sample[k] / 100 * 2 - 1  # [-1,1]
                # sample[k] = sample[k] / 394.9 * 2 - 1  # [-1,1]
                sample[k] = sample[k] / 30.9 * 2 - 1  # [-1,1]
