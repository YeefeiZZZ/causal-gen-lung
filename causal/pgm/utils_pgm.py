...
def plot_cf(
    ...
    for j in range(n):
        ...
        cf_a = str(np.round(cf_pa["wcc"][j].item(), 1))
...
def plot_cf_mimic(
    ...
    m = 3 if var_cf_x is None else 4  # nrows
    s = 4
    fig, ax = plt.subplots(m, n, figsize=(n*2 * s - 6, m * s), facecolor="white")
    # fig, ax = plt.subplots(m, n, figsize=(n*s, m*s+2))
    _, _ = plot(x, ax=ax[0])
    _, _ = plot(cf_x, ax=ax[1])
    _, _ = plot(
    ...
    sex_categories = ["female", "male"]  # 0,1
    race_categories = ["white","black", "asian"]  # 0,1,2
    findings_categories = ['no','effusion','pneumonia'] #0,1,2
    ...
    for j in range(n):
        ...
        ax[1, j].set_title(msg + r"$)$", fontsize=fs - 16, pad=pad + 4)
        ...
        cf_wcc = str(np.round(cf_pa["wcc"][j].item(), 1))
    ...
    if is_train == True:
        ...
        tr = 'val'
    # print(tr)
    base_dir = f"/mnt/de1dcd1c-9be8-42ed-aa06-bb73570121ac/cf_output_60k/{tr}_epoch{epoch}step_{step}"
    ...
    for j in range(n):
        ...
        cf_img_filename = f"{j}_cf_{cf_s}_{cf_m}_{cf_a}_{cf_wcc}_{cf_f}.png"
