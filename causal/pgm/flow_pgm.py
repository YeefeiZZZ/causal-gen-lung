...
class ChestPGM(BasePGM):
    def __init__(self, args: Hparams):
        ...
        for k in ["a","wcc", "f"]:
            ...
            self.register_buffer(f"{k}_base_scale", torch.ones(1))
        # age spline flow
        self.wcc_flow_components = T.ComposeTransformModule([T.Spline(1)])
        # self.age_constraints = T.ComposeTransform([
        #     T.AffineTransform(loc=4.09541458484, scale=0.32548387126),
        #     T.ExpTransform()])
        self.wcc_flow = T.ComposeTransform(
            [
                self.wcc_flow_components,
        ...
        # Finding (conditional) via MLP, (a, wcc) -> fss
        finding_net = DenseNN(2, [8, 16], param_dims=[3], nonlinearity=nn.Softmax())
        ...
        if args.setup != "sup_pgm":
            ...
            # q(wcc | x, f) ~ Normal(mu(x), sigma(x))
            self.encoder_wcc = ResNet18(num_outputs=2, context_dim=3, **kwargs)
            self.f_wcc = (
                lambda x: args.std_fixed * torch.ones_like(x)
                if args.std_fixed > 0
                else F.softplus(x)
    ...
    def model(self) -> Dict[str, Tensor]:
        ...
        # p(wcc), wcc flow
        pwcc_base = dist.Normal(self.wcc_base_loc, self.wcc_base_scale).to_event(1)
        pwcc = dist.TransformedDistribution(pwcc_base, self.wcc_flow)
        wcc = pyro.sample("wcc", pwcc)
        # age_ = self.age_constraints.inv(age)
        _ = self.age_flow_components  # register with pyro
        _ = self.wcc_flow_components  # register with pyro
        ...
        # p(f | a, wcc), finding as OneHotCategorical conditioned on age and wcc
        finding_dist_base = dist.OneHotCategorical(logits=self.finding_logits) #.to_event(1)
        finding_dist = ConditionalTransformedDistributionGumbelMax(
            finding_dist_base, [self.finding_transform_GumbelMax]).condition(torch.cat([age, wcc], dim=1))
    ...
    def guide(self, **obs) -> None:
        with pyro.plate("observations", obs["x"].shape[0]):
            ...
            # q(wcc | x, f)
            if obs["wcc"] is None:
                wcc_loc, wcc_logscale = self.encoder_wcc(obs["x"], y=obs["finding"]).chunk(
                    2, dim=-1
                )
                qwcc_xf = dist.Normal(wcc_loc, self.f_wcc(wcc_logscale)).to_event(1)
                pyro.sample("wcc_aux", qwcc_xf)
            # q(a | x, f)
            if obs["age"] is None:
                a_loc, a_logscale = self.encoder_a(obs["x"], y=obs["finding"]).chunk(
                    2, dim=-1
                )
                qa_xf = dist.Normal(a_loc, self.f_a(a_logscale)).to_event(1)
    ...
    def model_anticausal(self, **obs) -> None:
        ...
        with pyro.plate("observations", obs["x"].shape[0]):
            ...
            pyro.sample("wcc_aux", qwcc_xf, obs=obs["wcc"])
    ...
    def predict(self, **obs) -> Dict[str, Tensor]:
        ...
        f_prob = F.softmax(self.encoder_f(obs["x"]),dim=-1)
        # q(wcc | x, f)
        wcc_loc, _ = self.encoder_wcc(obs["x"], y=obs["finding"]).chunk(2, dim=-1)
        # q(a | x, f)
