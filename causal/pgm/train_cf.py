...
def cf_epoch(
    ...
    stats = {k: 0 for k in ["loss", "D_loss", "aux_loss", "elbo", "nll", "kl", "n"]}
    # stats = {k: 0 for k in ["loss", "cross entropy", "aux_loss", "elbo", "nll", "kl", "n"]}
    # stats = {k: 0 for k in ["loss", "f1", "aux_loss", "elbo", "nll", "kl", "n"]}
    steps_skipped = 0
    ...
    for i, batch in loader:
        ...
        if is_train:
            ...
            if i < 1000 or grad_norm < avg_grad_norm * 0.6:
                optimizer.step()
                lagrange_opt.step()
                model.lmbda.data.clamp_(min=0)
                ema.update()
        ...
        stats["D_loss"] += out["D_loss"] * bs
        # stats["cross entropy"] += out["cross entropy"] * bs
        # stats["f1"] += out["f1"] * bs
    ...
    if not args.testing:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if epoch % args.eval_freq == 0:
                ...
                if metrics["finding_rocauc"] > args.best_loss:
                    # args.best_loss = valid_stats["loss"]
                    args.best_loss = metrics["finding_rocauc"]
