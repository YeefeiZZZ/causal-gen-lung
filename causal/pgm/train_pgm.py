...
def get_free_device_with_nvidia_smi():
    ...
    free_device_id = max(range(len(free_memories)), key=lambda i: free_memories[i])
    return free_device_id

# free_device_id = get_free_device_with_nvidia_smi()
free_device_id = 0
# print(f"Using CUDA device: {free_device_id}")
...
def preprocess(
    ...
    if "x" in batch.keys():
        batch["x"] = (batch["x"].float().cuda(free_device_id) - 127.5) / 127.5  # [-1,1]
    ...
    for k in not_x:
        if split == "u":  # unlabelled
            ...
            batch[k] = batch[k].float().cuda(free_device_id)
    ...
    if "ukbb" in dataset:
        for k in not_x:
            if k in ["age", "wcc", "brain_volume", "ventricle_volume"]:
                # print(k,"yes",batch[k])
    ...
    for k in model.variables.keys():
        if "ukbb" in args.dataset:
            ...
            if k in ["sex"]:
                ...
                preds_k = (preds[k] + 1) * 15.45 # unormalize
                targets_k = (targets[k] + 1) * 15.45 # unormalize
                stats[k + "_mae"] = (targets_k - preds_k).abs().mean().item()
...
if __name__ == "__main__":
    ...
    model.cuda(free_device_id)
    ema.cuda(free_device_id)
