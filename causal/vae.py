...
class Encoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Dict[int, Tensor]:
        self.stem = self.stem.to(free_device_id)  # 确保网络层与输入数据在同一设备
        x = self.stem(x.cuda(free_device_id))
        acts = {}
        self.blocks = self.blocks.to(free_device_id) 
...
class DGaussNet(nn.Module):
    ...
    def nll(self, h: Tensor, x: Tensor) -> Tensor:
        ...
        centered_x = x.cuda(free_device_id) - loc.cuda(free_device_id)
...
class HVAE(nn.Module):
    ...
    def forward(self, x: Tensor, parents: Tensor, beta: int = 1) -> Dict[str, Tensor]:
        ...
        #print('Parents')
        #print(parents.shape)
        self.decoder = self.decoder.to(free_device_id) 
        ...
        self.likelihood = self.likelihood.to(free_device_id) 
        nll_pp = self.likelihood.nll(h.cuda(free_device_id), x.cuda(free_device_id))
        if self.free_bits > 0:
